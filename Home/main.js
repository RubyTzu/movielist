...
function renderPaginator(amount) {
    ...
    const numberOfPages = Math.ceil(amount / 12)
    let rawHTML = ''
...
function getMoviesByPage(page) {
    ...
    const pageContent = SearchMovies.length ? SearchMovies : movies
    const startMovie = (page - 1) * 12
    ...
    if (cardStyle.matches('.switch-style-btn-click')) {
        ...
        for (let i = 0; i < CurrentPageMoviesNum; i++) {
            ...
            for (let favMovie of favoriteMovies) {
                if ((Number(chooseBtn.dataset.id) === favMovie.id)) {
...
function removeFromFavorite(id) {
    const favoriteContent = SearchMovies.length ? SearchMovies : favoriteMovies
    const removeIndex = favoriteMovies.findIndex(element => element.id === id)

    favoriteContent.splice(removeIndex, 1)
...
dataPanel.addEventListener('click', function onClickedPanel(e) {
    ...
    } else if (e.target.matches('.favorite-btn') && (e.target.classList.contains('favorite-btn-click'))) {
        e.target.classList.toggle('favorite-btn-click')
